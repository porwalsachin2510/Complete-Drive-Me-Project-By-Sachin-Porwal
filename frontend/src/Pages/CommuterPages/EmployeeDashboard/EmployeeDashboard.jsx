...
export default function EmployeeDashboard() {
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'my-bookings':
                return <MyBookingsTab bookings={myBookings} onCancel={handleCancelBooking} loading={loading} />;
            case 'history':
                return <HistoryTab history={history} loading={loading} />;
            case 'notifications':
                return <NotificationsTab notifications={notifications} loading={loading} />;
            default:
                return <TripInfoTab tripInfo={tripInfo} loading={loading} />;
        }
    };
...
function TripInfoTab({ tripInfo, loading }) {
    if (loading) return <div className="loading">Loading trip information...</div>;
    if (!tripInfo) return <div className="empty-state">No trip information available</div>;
        ...
        <div className="tab-content">
            ...
            <div className="trip-info-cards">
                <div className="info-card">
                    <label>Route</label>
                    <p>{tripInfo.route?.fromLocation} → {tripInfo.route?.toLocation}</p>
                </div>
                <div className="info-card">
                    <label>Vehicle</label>
                    <p>{tripInfo.vehicle?.make} {tripInfo.vehicle?.model}</p>
                    ...
                    {bookings.map(booking => (
                        <div key={booking._id} className="booking-card">
                            <div className="booking-info">
                                ...
                                <p className="date">{new Date(booking.tripDate).toLocaleDateString()} at {booking.startTime}</p>
                ...
                <div className="history-list">
                    {history.map(trip => (
                        <div key={trip._id} className="history-item">
                            <div className="history-date">{new Date(trip.date).toLocaleDateString()}</div>
                            <div className="history-route">{trip.fromLocation} → {trip.toLocation}</div>
