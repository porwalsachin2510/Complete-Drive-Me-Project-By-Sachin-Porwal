...
function AdminSettlement() {
    ...
    const processMonthlySettlement = async () => {
        try {
            ...
            if (response.data.success) {
                alert(`Settlement calculated for ${response.data.settlements.length} partners`);
                fetchSettlements();
            }
        } catch (err) {
        ...
        <div className="admin-settlement">
            ...
            <div className="settlement-stats">
                ...
                <div className="stat-card commission">
                    ...
                    <div className="stat-value">{stats.totalCommission.toFixed(2)} AED</div>
                </div>
                        ...
                        <tbody>
                            {settlements.map((settlement) => (
                                <tr key={settlement.partnerId}>
                                    ...
                                    <td><span className="badge">{settlement.role}</span></td>
                                    <td className="amount">{settlement.balance.toFixed(2)}</td>
                                    <td className="amount pending">
                                        {settlement.pendingAmount.toFixed(2)}
                                    </td>
                                    <td className="amount">{settlement.commissionDebt.toFixed(2)}</td>
                                    <td className="amount">{settlement.totalEarnings.toFixed(2)}</td>
                                    <td className="date">
                    ...
                    <div className="payout-modal" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="modal-content">
                            <div className="form-group">
                                ...
                                <p className="info-text">Pending Amount: {selectedPayout.pendingAmount.toFixed(2)} AED</p>
                            </div>
